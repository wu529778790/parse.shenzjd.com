...
// RUN_LIVE_PARSE 由 npm run test:live 注入；WEIBO_NOTE_TEST 供单独调试本文件使用
...
function req(shareUrl: string) {
  return new Request(
    `http://127.0.0.1/api/weibo?url=${encodeURIComponent(shareUrl)}`,
...
describe.skipIf(!RUN)("微博链接真机解析", () => {
  for (const c of CASES) {
    it(c.name, async () => {
      const res = await GETWeibo(req(c.url));
